...
async def humanize_html_content(content: str, intensity: str = "medium") -> tuple[str, bool]:
    ...
    # 智能处理：保留 HTML 结构，只替换文本内容
    # 提取纯文本进行 AI 处理
    text_content = re.sub(r'<[^>]+>', '<<<SEP>>>', content)
    ...
    try:
        ...
        # 方案：用 humanized 后的纯文本 + 蓝色主题包装
        # 这样既有去痕效果，又有主题样式
        humanized_text = result.humanized.strip()
        
        # 用 markdown 转 HTML（用于段落结构）
        ...
        blue_css = """
        <style>
        .opc-article { font-family: -apple-system, BlinkMacSystemFont, "Helvetica Neue", "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei UI", "Microsoft YaHei", Arial, sans-serif; font-size: 16px; line-height: 1.8; color: #333; max-width: 100%; overflow-x: hidden; }
        .opc-article h1 { font-size: 24px; font-weight: bold; color: #3b8bba; margin: 20px 0 10px; text-align: center; border-bottom: 2px solid #3b8bba; padding-bottom: 10px; }
        .opc-article h2 { font-size: 20px; font-weight: bold; color: #fff; margin: 18px 0 8px; padding: 10px 16px; background: #3b8bba; border-radius: 4px; }
        .opc-article h3 { font-size: 18px; font-weight: bold; color: #3b8bba; margin: 16px 0 6px; border-bottom: 2px dotted #3b8bba; padding-bottom: 5px; }
        .opc-article p { margin: 12px 0; line-height: 1.8; }
        .opc-article blockquote { background: linear-gradient(135deg, #f0f7ff 0%, #fff 100%); border-left: 4px solid #3b8bba; padding: 12px 16px; margin: 16px 0; color: #555; border-radius: 0 8px 8px 0; }
        .opc-article table { width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 14px; border-radius: 8px; overflow: hidden; }
        .opc-article th { background: #3b8bba; color: #fff; padding: 12px 15px; text-align: left; }
        .opc-article td { padding: 12px 15px; border-bottom: 1px solid #e8f4fc; }
        .opc-article ul { list-style: none; padding: 0; margin: 12px 0; }
        .opc-article li { padding: 6px 0 6px 24px; position: relative; }
        .opc-article li::before { content: "•"; position: absolute; left: 8px; color: #3b8bba; }
        .opc-article strong { color: #3b8bba; font-weight: bold; }
        .opc-article em { color: #5bc0de; font-style: italic; }
        .opc-article hr { border: none; border-top: 2px dashed #3b8bba; margin: 20px 0; }
        .opc-article .author { text-align: center; color: #999; font-size: 13px; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; }
        </style>
        <div class="opc-article">
        """ + html_body + """
        </div>
        """

        return blue_css, True
